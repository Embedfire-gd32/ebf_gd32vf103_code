...
//返回值:0,成功;
//    其他,错误代码
static BOOL WM8978_Write_Reg(u8 reg,u16 val)
{
	pIIC->Open();

	pIIC->Start();
	if(!pIIC->WriteByte((WM8978_ADDR<<1)|0,IIC_ACK))//发送器件地址+写命令
	{
		pIIC->Stop();
...
void WM8978_ADDA_Cfg(u8 dacen,u8 adcen)
{
	...
	val=WM8978_Read_Reg(3);	//读取R3
	if(dacen)
	{
		val |= 3<<0;		//R3最低2个位设置为1,开启DACR&DACL
	}
	else
	{
		val &= ~(3<<0);		//R3最低2个位清零,关闭DACR&DACL.
	}
	WM8978_Write_Reg(3,val);	//设置R3

	val=WM8978_Read_Reg(2);	//读取R2
	if(adcen)
	{
		val |= 3<<0;		//R2最低2个位设置为1,开启ADCR&ADCL
	}
	else
	{
		val &= ~(3<<0);		//R2最低2个位清零,关闭ADCR&ADCL.
	}
	WM8978_Write_Reg(2,val);	//设置R2
}
//WM8978 输入通道配置 
//micen:MIC开启(1)/关闭(0)
