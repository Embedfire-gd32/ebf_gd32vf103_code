...
#define	MOVE_IN		1
#define	MOVE_OUT	2
extern void	GUI_DEMO_ShowWave2(void);
...
static LRESULT	WinProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch(msg)
	{
		case WM_CREATE: //窗口创建时,会自动产生该消息,在这里做一些初始化的操作或创建子窗口.
		{
			...
			OffsetRect(&rc,0,rc.h+8);
			CreateWindow(BUTTON,L"BTN-4",BS_NOTIFY|WS_VISIBLE,rc.x,rc.y,rc.w,rc.h,hwnd,eID_BTN4,NULL,NULL);
			...
			wnd=CreateWindow(LISTBOX,L"Listbox",WS_OVERLAPPED|WS_CAPTION|LBS_LINE|LBS_NOTIFY|WS_BORDER|WS_VISIBLE,8,20,200,150,hwnd,ID_LISTBOX1,NULL,NULL);
        ...
        GUI_DEMO_ShowWave();
			...
			if(id==eID_BTN4)
			{
				if(code==BN_PUSHED)
				{
				//	EnableDragWindow(hwnd,TRUE);
				}

				if(code==BN_CLICKED)
				{
				//	EnableDragWindow(hwnd,FALSE);
				}
			}
...
void	xGUI_DEMO_MultiWindow(void)
{
	HWND	hwnd;
	WNDCLASS	wcex;
	MSG msg;
	...
	hwnd	=CreateWindowEx(WS_EX_LOCKPOS,
								&wcex,
								L"GUI_DEMO: Multi-Window(Overlapped)", //窗口名称
								WS_OVERLAPPEDWINDOW,
