...
static LRESULT	WinProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch(msg)
	{
		...
		case WM_PAINT: //窗口需要绘制时，会自动产生该消息.
		{
			...
			X_MeterBackgnd(hdc_mem,rc.w>>1,rc.h>>1,r-6,tm.sec);

			EnableAntiAlias(hdc_mem,TRUE);
			X_MeterPointer(hdc_mem,rc.w>>1,rc.h>>1,(r*160)>>8,MapRGB(hdc_mem,220,220,50),90,360,360,(tm.hour%12)*30+(tm.min*30)/60,pt_type);
			X_MeterPointer(hdc_mem,rc.w>>1,rc.h>>1,(r*180)>>8,MapRGB(hdc_mem,100,150,220),90,360,60,tm.min,pt_type);
			X_MeterPointer(hdc_mem,rc.w>>1,rc.h>>1,(r*220)>>8,MapRGB(hdc_mem,255,20,20),90,360,60,tm.sec,pt_type);
			...
			x_wsprintf(wbuf,L"%02d:%02d:%02d",tm.hour,tm.min,tm.sec);
			rc0.x=0;
			rc0.y=0;
			rc0.w=rc.w;
			rc0.h=rc.h;
