...
static LRESULT	WinProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch(msg)
	{
		...
		case WM_NOTIFY: //WM_NOTIFY消息:wParam低16位为发送该消息的控件ID,高16位为通知码;lParam指向了一个NMHDR结构体.
		{
			...
			{
				if(CDlg_FileExpoler(hwnd,20,30,300,300,L"Open BMP File","B:",file_path))
				{
		...
		case WM_PAINT: //窗口需要绘制时，会自动产生该消息.
		{
			...
			if(1)
			{
				...
				if(buf!=NULL)
				{
					...
					if(BMP_GetInfo(&bm,buf))
					{
						BMP_Draw(hdc,(rc.w-(int)bm.Width)/2,(rc.h-(int)bm.Height)/2,buf,NULL);
					}
					...
					////
					x_wsprintf(wbuf,L"BMP Time: %dms",t1-t0);
