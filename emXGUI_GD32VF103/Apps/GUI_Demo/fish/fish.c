...
static void FishInit(int w,int h)
{
	disp_w =w;
	disp_h =h;
...
void FishExecu(HDC hdc)
{
	...
	ClrDisplay(hdc,NULL,MapRGB(hdc,0,10,30));
...
void DrawBitmap_Test_Fish(void)	
{
	...
	FishInit(ScreenW,ScreenH);

	pSurf =CreateSurface(BM_RGB565,ScreenW,ScreenH,0,NULL);
	//pSurf =CreateSurface(BM_RGB565,450,250,0,NULL);
