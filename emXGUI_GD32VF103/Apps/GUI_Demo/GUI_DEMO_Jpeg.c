...
static LRESULT	WinProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch(msg)
	{
		...
		case WM_NOTIFY: //WM_NOTIFY消息:wParam低16位为发送该消息的控件ID,高16位为通知码;lParam指向了一个NMHDR结构体.
		{
			...
			{
				if(CDlg_FileExpoler(hwnd,20,30,300,300,L"Open JPG File","B:",file_path))
				{
		...
		case WM_PAINT: //窗口需要绘制时，会自动产生该消息.
		{
			...
			if(1)
			{
				...
				if(jpg_m==0)
				{
					if(JPEG_Info(&w,&h,buf))
					{
						JPEG_Out(hdc,10,20,buf,fsize);
						//JPEG_Out(hdc,(rc.w-w)/2,(rc.h-h)/2,buf,fsize);
					}
				}

				if(jpg_m==1)
				{
					jdec =JPG_Open(buf,fsize);
					if(jdec!=NULL)
					{
						JPG_GetImageSize(&w,&h,jdec);
						JPG_Draw(hdc,10,20,jdec);
						//JPG_Draw(hdc,(rc.w-w)/2,(rc.h-h)/2,jdec);
						JPG_Close(jdec);
					}
				}
				...
				vfree(buf);
					...
					////
				x_wsprintf(wbuf,L"JPG Time: %dms",t1-t0);
