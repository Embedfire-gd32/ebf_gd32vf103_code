...
static void                   ILI9341_EXMC_Config         ( void );
...
/**
 ...
 void ILI9341_Write_Cmd ( uint16_t usCmd )
{
	* ( __IO uint16_t * ) ( EXMC_Addr_ILI9341_CMD ) = usCmd;
...
void ILI9341_Write_Data ( uint16_t usData )
{
	* ( __IO uint16_t * ) ( EXMC_Addr_ILI9341_DATA ) = usData;
...
uint16_t ILI9341_Read_Data ( void )
{
	return ( * ( __IO uint16_t * ) ( EXMC_Addr_ILI9341_DATA ) );
...
static void ILI9341_GPIO_Config ( void )
{
	...
	rcu_periph_clock_enable(ILI9341_RST_CLK);
	rcu_periph_clock_enable(ILI9341_BK_CLK);

	gpio_pin_remap_config(GPIO_EXMC_NADV_REMAP, ENABLE);    // 没有用到 EXMC_NADV 信号，重映射使其可以用于其他外设
  ...
  /* 配置LCD复位RST控制管脚*/
	gpio_init(ILI9341_RST_PORT, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, ILI9341_RST_PIN);
...
{
	if ( enumState )
		gpio_bit_reset ( ILI9341_BK_PORT, ILI9341_BK_PIN );
	else
		gpio_bit_set ( ILI9341_BK_PORT, ILI9341_BK_PIN );
...
{			
	gpio_bit_reset ( ILI9341_RST_PORT, ILI9341_RST_PIN );	 //低电平复位

	ILI9341_Delay ( 0xAFF ); 					   

	gpio_bit_set ( ILI9341_RST_PORT, ILI9341_RST_PIN );
