...
{ 
  ...
  /* enable GPIOB clock */
  rcu_periph_clock_enable(XPT2046_SPI_GPIO_CLK);

  gpio_init(XPT2046_SPI_CLK_PORT, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, XPT2046_SPI_CLK_PIN);
  gpio_init(XPT2046_SPI_MOSI_PORT, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, XPT2046_SPI_MOSI_PIN);
  gpio_init(XPT2046_SPI_MISO_PORT, GPIO_MODE_IPU, GPIO_OSPEED_50MHZ, XPT2046_SPI_MISO_PIN);
  gpio_init(XPT2046_SPI_CS_PORT, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, XPT2046_SPI_CS_PIN);

  gpio_init(XPT2046_PENIRQ_GPIO_PORT, GPIO_MODE_IPU, GPIO_OSPEED_50MHZ, XPT2046_PENIRQ_GPIO_PIN);
...
void Calibrate_or_Get_TouchParaWithFlash(uint8_t LCD_Mode,uint8_t forceCal)
{
	...
	//读取触摸参数标志
	spi_flash_buffer_read(&para_flag,FLASH_TOUCH_PARA_FLAG_ADDR,1);
	...
	{ 		
		...
		if(  para_flag == FLASH_TOUCH_PARA_FLAG_VALUE && forceCal == 1)
		{
			spi_flash_buffer_read((uint8_t *)&strXPT2046_TouchPara,FLASH_TOUCH_PARA_ADDR,4*6*8);
		}
		...
		//写入触摸参数标志
		spi_flash_buffer_write(&para_flag,FLASH_TOUCH_PARA_FLAG_ADDR,1);
		//写入最新的触摸参数
		spi_flash_buffer_write((uint8_t *)&strXPT2046_TouchPara,FLASH_TOUCH_PARA_ADDR,4*6*8);
