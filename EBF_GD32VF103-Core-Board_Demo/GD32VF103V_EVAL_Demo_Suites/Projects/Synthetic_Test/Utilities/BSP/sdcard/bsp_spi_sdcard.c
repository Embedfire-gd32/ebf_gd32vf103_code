...
static void GPIO_Configuration(void)
{
  ...
  SD_SPI_Clock_FUN(SD_SPI_CLK);
  rcu_periph_clock_enable(RCU_GPIOB);

  /* 配置 CLK MOSI */
  gpio_init(SD_SPI_SCK_GPIO_PORT, GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ, SD_SPI_SCK_PIN | SD_SPI_MOSI_PIN);    // 这里 CLK 与 MOSI 是同一个端口
  gpio_init(SD_SPI_MISO_GPIO_PORT, GPIO_MODE_IN_FLOATING, GPIO_OSPEED_50MHZ, SD_SPI_MISO_PIN);
  ...
  spi_init(SD_SPI, &spi_init_struct);

  /* set crc polynomial */
  spi_crc_polynomial_set(SD_SPI, 7);
  /* enable SPI1 */
  spi_enable(SD_SPI);
...
uint8_t SD_WriteByte(uint8_t Data)
{
  /* loop while data register in not emplty */
    while (RESET == spi_i2s_flag_get(SD_SPI,SPI_FLAG_TBE));

    /* send byte through the SPI0 peripheral */
    spi_i2s_data_transmit(SD_SPI,Data);

    /* wait to receive a byte */
    while(RESET == spi_i2s_flag_get(SD_SPI,SPI_FLAG_RBNE));

    /* return the byte read from the SPI bus */
    return(spi_i2s_data_receive(SD_SPI));
}
