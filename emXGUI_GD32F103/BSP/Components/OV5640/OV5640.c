...
static const u16 OV5640_jpeg_reg_tbl[][2]=
{
...
static const u16 ov5640_rgb565_reg_tbl[][2]=
{
	0x4300, 0X6F,
	0X501F, 0x01,
	// 1280x800, 15fps
...
//OV5640初始化寄存器序列表
static const u16 ov5640_init_reg_tbl[][2]=
{
...
//OV5640写寄存器
//返回值:0,成功;1,失败.
static BOOL OV5640_WR_Reg(u16 reg,u8 val)
{
	pIIC->Open();

	pIIC->Start();
	if(!pIIC->WriteByte(OV5640_WT_ADDR,IIC_ACK))//发送器件地址+写命令
	{
		pIIC->Stop();
	...
	pIIC->WriteByte((reg>>8),IIC_ACK);//写寄存器地址H
...
//OV5640读寄存器
//返回值:读到的寄存器值
static u8 OV5640_RD_Reg(u16 reg)
{
	u8 val=0;
	...
	pIIC->Start();
	if(!pIIC->WriteByte(OV5640_WT_ADDR,IIC_ACK))	//发送写命令
	{
		goto exit;
	}

	pIIC->WriteByte(reg>>8,IIC_ACK);   		//发送低8位地址
	pIIC->WriteByte(reg&0XFF,IIC_ACK);   	//发送低8位地址

	pIIC->Start();
	pIIC->WriteByte(OV5640_RD_ADDR,IIC_ACK);   	//发送读命令
...
static BOOL OV5640_WR_Reg(u16 reg,u8 val)
{
    ...
    if(!SWI2C_WriteByte(OV5640_WT_ADDR,I2C_Ack)) 	goto exit;
...
static u8 OV5640_RD_Reg(u16 reg)
{
	...
	SWI2C_SendStart();
	SWI2C_WriteByte(OV5640_RD_ADDR,I2C_Ack);
...
void OV5640_Start(const struct i2c_ops *iic)
{
	/* Enable GPIO Clock */
  //  outpw(REG_CLK_PCLKEN0,inpw(REG_CLK_PCLKEN0)|(1<<3));
    /* switch I2C pin function, to do... */
  //  SWI2C_Open(eDRVGPIO_GPIOB,eDRVGPIO_PIN0,eDRVGPIO_GPIOB,eDRVGPIO_PIN1,NULL);
	...
	if(OV5640_Init())
	{
		...
		OV5640_Color_Saturation(4);
		OV5640_Exposure(3);
		OV5640_Brightness(4);
		OV5640_Contrast(2);
		OV5640_Sharpness(100);

		//OV5640_Test_Pattern(1);

		OV5640_ImageWin_Set(0,0,800*1,600*1);
		OV5640_OutSize_Set(0,0,800,600);
