	...
	switch(msg)
	{
		...
		case WM_NOTIFY: //WM_NOTIFY消息:wParam低16位为发送该消息的控件ID,高16位为通知码;lParam指向了一个NMHDR结构体.
		{
			...
			if(id==eID_BTN3 && code==BN_CLICKED) // 按钮“单击”了.
			{
				...
				MoveWindow(hwnd,0,0,rc.w,rc.h,TRUE);
			}
		}
		break;
		////
...
//			SetTimer(hwnd,1,20,TMR_START,NULL);
			...
			rc.x =10;
			rc.y =10;
			CreateWindow(&wcex_dlg,L"Win1",WS_CLIPSIBLINGS|WS_DLGFRAME|WS_BORDER|WS_VISIBLE,rc.x,rc.y,rc.w,rc.h,hwnd,eID_WIN1,NULL,(LPVOID)&win1_data);

			OffsetRect(&rc,0,rc.h+8);
			CreateWindow(&wcex_dlg,L"Win2",WS_CLIPSIBLINGS|WS_DLGFRAME|WS_BORDER|WS_VISIBLE,rc.x,rc.y,rc.w,rc.h,hwnd,eID_WIN2,NULL,(LPVOID)&win2_data);

			OffsetRect(&rc,0,rc.h+8);
			CreateWindow(&wcex_dlg,L"Win3",WS_CLIPSIBLINGS|WS_DLGFRAME|WS_BORDER|WS_VISIBLE,rc.x,rc.y,rc.w,rc.h,hwnd,eID_WIN3,NULL,(LPVOID)&win3_data);

			OffsetRect(&rc,0,rc.h+8);
			CreateWindow(&wcex_dlg,L"Win4",WS_CLIPSIBLINGS|WS_DLGFRAME|WS_BORDER|WS_VISIBLE,rc.x,rc.y,rc.w,rc.h,hwnd,eID_WIN4,NULL,(LPVOID)&win4_data);

			wnd=CreateWindow(LISTBOX,_T("Listbox"),LBS_LINE|LBS_NOTIFY|WS_BORDER|WS_VISIBLE,10,100,200,150,hwnd,ID_LISTBOX1,NULL,NULL);
